...
export async function ensureLocation(accessToken, key, postalCode) {
  ...
  if (r.ok || r.status === 204 || r.status === 409) return; // 409 = already exists
  throw new Error(`location ${r.status}: ${(await r.text()).slice(0, 160)}`);
}
...
export async function putInventoryItem(accessToken, sku, item) {
  ...
  if (!r.ok && r.status !== 204) {
    throw new Error(`inventory ${r.status}: ${(await r.text()).slice(0, 250)}`);
  }
}
...
export async function createOffer(accessToken, offer) {
  ...
  if (!r.ok) throw new Error(`offer ${r.status}: ${JSON.stringify(d).slice(0, 250)}`);
  return d.offerId;
}
