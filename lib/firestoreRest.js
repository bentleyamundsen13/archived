...
// List the documents in a collection path (e.g. "collections/cid/items").
export async function listDocsRest(path, pageSize = 300) {
  const token = await googleAccessToken();
  const resp = await fetch(`${docBase()}/${path}?pageSize=${pageSize}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!resp.ok) return [];
  const data = await resp.json();
